...
- Click "New Prompt Template"
  ...
  - Name: Gift Finder Self Grouping
  - API Name: Gift_Finder_Self_Grouping
  - Sources:
    - Name: User Query
    - API Name: User_Query
...
```
Your task is to recommend products that best match a natural language query performed by a shopper on an online store who is looking for gifts.

The gifts must be selected from the list of products below, in CSV format, where the first line are the headers:
...
Processing steps:
1. Find products that strictly match the shopper's query.
2. Within the set of matching products, find 3 characteristics and sort the products according to those characteristics. I can be type of product, color, fit, or anything else.
3. Output the products in a valid JSON object as described in the schema below, with no other text before or after the JSON.

Requirements:
- If you do do find products that strictly match the shopper's query, do not output any products. For example, if a shopper requests accessories, only return accessories. If the shopper requests tops, only return tops.
- Return a maximum of 8 products per group.
- Do not output anything before or after the JSON object.
- IMPORTANT! Output only product IDs from the CSV above. Do not output product IDs that are not in the CSV.

JSON Schema:
{
"groupOne":{
"groupName": <short name for the logic grouping these products together>,
"products": <array of product IDs>,
"text": <summary explaining how the products match the query>
},
"groupTwo":{
"groupName": <short name for the logic grouping these products together>,
"products": <array of product IDs>,
